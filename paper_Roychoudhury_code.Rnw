...
Given the significance level $\alpha$, the null value (NV) and the decision value (DV) for $\log$ HR, we can calculate the minimum sample size (for normally distributed data). Notice here that the DV $<$ NV, $\log$ DV $<$ $\log$ NV.
\begin{equation*}
...
Prior: minimally informative unimodal beta prior distribution $\text{Beta}(0.0811,1)$, which has mean 0.075.\\
Because of the absence of a comparator (in single arm trials), NV is set to 7.5\% rather than 0, based on a literature review and clinical discussions.\\
A minimum improvement of 10\% is considered necessary for further development, so DV is set to be 10\%+7.5\%=17.5\%. Notice in this example, the NV < DV.\\
So the dual-criterion is:
\begin{enumerate}
...
@
\end{center}
Additionally, we explore how the minimum sample size changes according to different values of $\alpha$. The below plot gives the relationship between them.
\begin{center}
<<prior_sample_size2, fig.show='asis', echo=FALSE>>=
# Next plot the minimum sample size VS alpha

# alpha_vector <- seq(0.1,1,0.1)
# n_min_vector <- rep(0, length(alpha_vector))
# par(mfrow=c(1,1))
...
#                       post.est = "median")
#   n_min_vector[i] <- n$n.min[[1]]
# }
# n_min_vector
# 
# 
alpha_vector <- seq(0.005,1,0.005)
n_min_vector_stored <- c(21,21,21,21,21,21,21,21,21,21,21,21,21,21,21,
                         22,22,22,22,22,22,22,22,22,22,22,22,22,22,22,
                         22,22,22,22,22,22,22,22,22,22,22,22,22,22,22,
                         22,22,22,22,22,22,22,22,22,17,17,17,23,23,23,
                         23,23,23,23,23,23,23,23,23,23,23,23,23,23,23,
                         23,23,23,23,23,23,23,17,17,17,17,17,18,18,18,
                         18,18,18,18,18,18,18,18,18,18,24,24,24,24,24,
                         24,24,24,24,24,18,18,18,18,18,18,18,18,18,18,
                         ...
                         20,20,20,20,20)
plot(alpha_vector,n_min_vector_stored, type="l",
     xlab="alpha", ylab="n_min", las=1)
@
\end{center}
