...
\begin{equation*}
n_{min}=\frac{\sigma^2 \times z_{\alpha}^2}{(\text{NV}-\text{DV})^2}
\end{equation*}
where $\sigma$ is the outcome standard deviation, and takes the value 2 under equal randomization for the standard normal approximation to time-to-event data. The $z_{\alpha}$ is the $100(1-\alpha)\%$ quantile of the standard normal distribution. It is the minimum sample size that implies statistical significance if the effect estimate equals the DV. This value is calculated under the situation that both criterion are just satisfied. As illustrated in the below graph, when the effect estimate $\theta=$ DV, and the lower bound of the confidence interval just touches the NV so that statistical significance is reached, we have the minimum sample size. Notice that when sample size equals the minimum sample size, there can only be GO or NO-GO decisions. 
\begin{center}
...
\end{equation*}
The probability of making a ``GO'' decision is the probability of the estimate smaller than the decision value and 
<<Figure 1, eval=FALSE, echo=TRUE, results=hide, fig=TRUE, highlight=TRUE>>=
# a sequence of true hazard ratios in log scale.
t.d <- log(seq(0.5, 1, 0.01))

# Left panel: sample size = 309
n <- 309
sd <- sqrt((2^2)/n) #sigma assumed to be 2. 
cut.ssig <- log(0.8) # statistical significance 
cut.crel <- log(0.8) # clinical relevance DV=0.8
pp.go <- pnorm(cut.crel, t.d, sd)
pp.ngo <- 1- pnorm(cut.ssig, t.d, sd)
pp.intd <- 1 -pp.go - pp.ngo

# Right panel: sample size = 420 
n <- 420
sd <- sqrt((2^2)/n) #sigma assumed to be 2
#cut.ssig <- log(0.83) # statistical significance NV=1????
cut.ssig <- log(1-qnorm(0.975)*sqrt(2^2/420))
cut.crel <- log(0.8) # critical relevance
...
plot(exp(t.d), pp.go, type="l", xlab= "Hazard Ratio", ylab="Probability", 
     main="n=309", col="green", ylim=c(0,1),xlim=c(0.5,1))
lines(exp(t.d), pp.ngo, type="l", col="red")
lines(exp(t.d), pp.intd, type="l", col="darkgoldenrod1")
abline(v=0.8, lty=1)

plot(exp(t.d), pp.go6, type="l", xlab= "Hazard Ratio", ylab="Probability", 
     main="n=420", col="green", ylim=c(0,1),xlim=c(0.5,1))
lines(exp(t.d), pp.ngo6, type="l", col="red")
lines(exp(t.d), pp.intd6, type="l", col="darkgoldenrod1")
abline(v=0.8, lty=1)
@
...
# Left panel data (n = 309)
n1 <- 309
sd1 <- sqrt((2^2) / n1)  # sigma assumed to be 2
cut.ssig1 <- log(0.8)    # statistical significance 
cut.crel1 <- log(0.8)    # clinical relevance DV=0.8
pp.go1 <- pnorm(cut.crel1, t.d, sd1)
pp.ngo1 <- 1 - pnorm(cut.ssig1, t.d, sd1)
pp.intd1 <- 1 - pp.go1 - pp.ngo1

# Right panel data (n = 420)
n2 <- 420
sd2 <- sqrt((2^2) / n2)  # sigma assumed to be 2
cut.ssig2 <- log(1 - qnorm(0.975) * sqrt(2^2 / 420)) # statistical significance 
cut.crel2 <- log(0.8)  # clinical relevance
pp.go2 <- pnorm(cut.crel2, t.d, sd2)
pp.ngo2 <- 1 - pnorm(cut.ssig2, t.d, sd2)
pp.intd2 <- 1 - pp.go2 - pp.ngo2

# Data for plotting
df1 <- data.frame(HazardRatio = exp(t.d), GO = pp.go1, NOGO = pp.ngo1, 
                  Inconclusive = pp.intd1)
df2 <- data.frame(HazardRatio = exp(t.d), GO = pp.go2, NOGO = pp.ngo2, 
                  Inconclusive = pp.intd2)

# Left panel plot (n = 309)
p1 <- ggplot(df1, aes(x = HazardRatio)) +
  geom_line(aes(y = GO, color = "GO")) +
  geom_line(aes(y = NOGO, color = "NOGO")) +
  geom_line(aes(y = Inconclusive, color = "Inconclusive")) +
  geom_vline(xintercept = c(0.8, 1.0), linetype = "dashed", color = "black") +  
  labs(x = "Hazard Ratio", y = "Probability", title = "n = 309") +
  scale_color_manual(values = c("GO" = "green", "NOGO" = "red", 
                                "Inconclusive" = "darkgoldenrod1")) +
  theme_minimal() +
  theme(legend.position = "bottom")

# Right panel plot (n = 420)
p2 <- ggplot(df2, aes(x = HazardRatio)) +
  geom_line(aes(y = GO, color = "GO")) +
  geom_line(aes(y = NOGO, color = "NOGO")) +
  geom_line(aes(y = Inconclusive, color = "Inconclusive")) +
  geom_vline(xintercept = c(0.8, 1.0), linetype = "dashed", color = "black") +
  labs(x = "Hazard Ratio", y = "Probability", title = "n = 420") +
  scale_color_manual(values = c("GO" = "green", "NOGO" = "red", 
                                "Inconclusive" = "darkgoldenrod1")) +
  theme_minimal() +
  theme(legend.position = "bottom")
...
# Adjust widths to make the plots wider

# Extract legend from one of the plots
legend <- get_legend(p1)

# Final plot with shared legend at the bottom
...
Also, the calculation of $\hat{\theta} = 0.736, 0.708, 0.659, 0.761$ should be looked into when the correct $\sigma$ is decided. The current understanding is that these values are calculated when the estimate $\hat{\theta} > \text{DV}$ and the confidence interval just touches 1, i.e. NO-GO is implied.
<< echo = TRUE >>=
...
sd1 <- sqrt((2^2)/n1) #sigma assumed to be 2?
#sqrt((1.723522^2)/n1)
cut.ssig <- log(0.736) # statistical significance NV=1
...
# Table3 <- rbind(subtable1[6:11,],subtable2[6:11,],subtable3[6:11,], 
# subtable4[6:11,], subtable5[6:11,])
...
plot(t.d, pp.go1, type="l", xlab= "Hazard Ratio", ylab="Probability", 
     main="n=70", col="green", ylim=c(0,1))
...
plot(t.d, pp.go2, type="l", xlab= "Hazard Ratio", ylab="Probability", 
     main="n=52", col="green", ylim=c(0,1))
