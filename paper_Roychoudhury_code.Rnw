...
df1 <- data.frame(HazardRatio = exp(t.d), GO = pp.go1, NOGO = pp.ngo1, Inconclusive = pp.intd1)

# Right panel (n = 420)
n2 <- 420
sd2 <- sqrt((2^2) / n2)
cut.ssig2 <- log(1) - qnorm(0.975) * sqrt(2^2 / 420)
cut.crel2 <- log(0.8)
pp.go2 <- pnorm(cut.crel2, t.d, sd2)
pp.ngo2 <- 1 - pnorm(cut.ssig2, t.d, sd2)
pp.intd2 <- 1 - pp.go2 - pp.ngo2
df2 <- data.frame(HazardRatio = exp(t.d), GO = pp.go2, NOGO = pp.ngo2, Inconclusive = pp.intd2)
...
Attempt to reproduce the values in Table 3. The main problem is to find the correct value for $\sigma$. There seem to be no detailed information about the choice of value for $\sigma$. We chose it to be 2 according to page 455 in the paper. \\
...
#sqrt((1.723522^2)/n1)
# cut.ssig <- 0-qnorm(0.9)* sqrt(2^2 / 70)
cut.ssig <- log(exp(0-qnorm(0.9)* sqrt(2^2 / n1))) # statistical significance NV=1
...
cut.ssig <- log(0.708)
cut.crel <- log(0.708)
cut.ssig <- qnorm(0.9,log(0.5),sd3) # different from 0.708
cut.crel <- qnorm(0.9,log(0.5),sd3)
...
n4 <- 38
sd4 <- sqrt((2^2)/n4)
cut.ssig <- log(exp(0-qnorm(0.9)* sqrt(2^2 / n4))) # different from 0.659
cut.crel <- log(exp(0-qnorm(0.9)* sqrt(2^2 / n4)))
...
n5 <- 38
sd5 <- sqrt((2^2)/n5)
cut.ssig <- log(exp(0-qnorm(0.8)* sqrt(2^2 / n5))) 
# How is the power guaranteed to be 0.1? This is only using alpha=0.2.
cut.crel <- log(exp(0-qnorm(0.8)* sqrt(2^2 / n5)))
