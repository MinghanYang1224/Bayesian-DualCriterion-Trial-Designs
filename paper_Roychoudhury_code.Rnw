...
plot(1:6, type = "n", xaxt = "n", ylim = c(0.2, 0.5), yaxt = "n", ylab = "", xlab = "Hazard Ratio", xlim=c(-2,12))
...
\begin{center}
<<Figure_1, results='hide', fig.height=5>>=
...
<< Table 3, echo = TRUE, results='asis'>>=
# minimum sample size
...
Prior: minimally informative unimodal beta prior distribution $\text{Beta}(0.0811,1)$, which has mean 0.75.\\
...
A minimum improvement of 10\% is considered necessary for further development, so DV is set to be 10\%+7.5\%=17.5\%.\\
So the dual-criterion is:
\begin{enumerate}
...
For this dual-criterion, the minimally required sample size is 22. For $n \geq 22$, clinical relevance ensures statistical significance. \\
For a Beta(a,b) prior, if we observe a number of successes (r) and failures (nn - r)
\begin{center}
<<Figure_2, echo=FALSE, results='hide', fig.width=5>>=
#This function calculates the minimal sample size for Bayesian DC design with binary endpoint
...
)
{
  ...
  if (post.est == "mean") { # if estimate posterior mean
    ...
    est = (a+rr)/(a+b+n) # what does it do?
  }
  ...
  if (post.est == "median") {
    ...
    for ( nn in n) {
      r = 0:nn
      est1= qbeta(0.5,a+r,b+nn-r) # the median of beta distribution
      rr[nn] = r[est1>=DV][1]
    }
    print(cbind(rr,n))
  ...
  # find n such that posterior prob >= p.positive 
  post.crit <- 1-pbeta(NV,a+rr,b+n-rr)
  post.crit.index = post.crit >= p.positive

  # find n such that for all m>=n, statistical criterion is fulfilled
  find.n =  cbind( n, rev(post.crit.index), 
                   cumsum(rev(post.crit.index))==n, rev(n))
  colnames(find.n) = c("index","okay","all.next.okay","n")

  n.min=NULL

  if (any(find.n[,3]==1) ) {
    n.okay = find.n[find.n[,3]==1,,drop=FALSE]
    n.min = n.okay[nrow(n.okay),4]
    n.min.10 = n.min+10
    est=round(est,3)
    ss.table = data.frame( n=n[1:n.min.10], # sample size
                           r=rr[1:n.min.10], # number of responders
                           p.obs=p.obs[1:n.min.10], #
                           est = est[1:n.min.10], # posterior estimate of median (by default)
                           post.p.positive=post.crit[1:n.min.10], # posterior probability of a positive effect
...
plot( 1:(n.min+10), post.crit[1:(n.min+10)], type="p", xaxs="i", xlab="n",
        ylab="pr(p>NV)",xlim = c(0,n.min+10), ylim=c(0.5,1),
        lwd=1,
        #col = ifelse(post.crit.index,"green","red"),
        pch = ifelse(post.crit.index,16,1),
