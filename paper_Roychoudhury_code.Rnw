...
\begin{abstract}
Related to paper ``Beyond p-values: A phase II dual-criterion design with statistical significance and clinical relevance'' \citet{Roychoudhury2018}. It addresses Bayesian dual-criterion design, sample size calculation, operational characteristics, and two cases: a randomized PoC design with time-to-event data and a single-arm PoC design with binary data. It also explores how the prior choices affect the minimum sample size. 
\end{abstract}
